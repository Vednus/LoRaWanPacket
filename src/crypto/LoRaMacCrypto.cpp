 ...
 * Jäckle ( STACKFORCE )
 *
 * LoRaMacCrypto.h was modify to work with AVR microcontroler thas have small
 * RAM.
 *
...
void LoRaMacJoinComputeMic(uint8_t *data, uint8_t len, uint8_t *key,
                           uint32_t *mic) {
    uint8_t X[16];
    uint8_t Y[16];
    ...
    // Only 4 bytes are returned (32 bits), which is less than the RFC
    // recommends. We return by appending 4 bytes to data, so there must be
    // space in data array.
    //
    ...
    *mic = (uint32_t)((uint32_t)Y[3] << 24 | (uint32_t)Y[2] << 16 |
...
void LoRaMacJoinDecrypt(const uint8_t *data, uint8_t len, const uint8_t *key,
                        uint8_t *decBuffer) {
...
void LoRaMacJoinComputeSKeys(uint8_t *key, uint8_t *appNonce, uint16_t devNonce,
...
void LoRaMacComputeMic(uint8_t *data, uint8_t len, uint8_t *key,
                       uint32_t address, uint8_t dir, uint32_t count,
                       uint32_t *mic) {
    uint8_t Block_B[16];
    uint8_t X[16];
    uint8_t Y[16];
...
void LoRaMacPayloadEncrypt(uint8_t *data, uint8_t len, uint8_t *key,
                           uint32_t address, uint8_t dir, uint32_t count) {
    uint8_t i, j;
    uint8_t Block_B[16];
    ...
    uint8_t restLength =
        len % 16;  // We work in blocks of 16 bytes, this is the rest
...
void LoRaMacPayloadDecrypt(uint8_t *data, uint8_t len, uint8_t *key,
                           uint32_t address, uint8_t dir, uint32_t count) {
...
void JoinComputeSKeys(uint8_t *key, uint8_t *appNonce, uint16_t devNonce,
                      uint8_t *nwkSKey, uint8_t *appSKey) {
    uint8_t *pDevNonce = (uint8_t *)&devNonce;
...
// cmac = aes128_encrypt(K, Block_A[i])
// ----------------------------------------------------------------------------
uint8_t PayloadEncode(uint8_t *buf, uint8_t len, uint8_t *key, uint8_t *dev,
                      uint32_t count, uint8_t dir) {
    uint8_t i, j;
    uint8_t Block_A[16];
    uint8_t bLen = 16;  // Block length is 16 except for last block in message
    ...
    // return(numBlocks*16);     // Do we really want to return all 16 bytes in
    // lastblock
...
//
// ----------------------------------------------------------------------------
uint8_t PayloadComputeMic(uint8_t *data, uint8_t len, uint8_t *key,
                          uint32_t count, uint8_t dir) {
    uint8_t Block_B[16];
    uint8_t X[16];
    uint8_t Y[16];
