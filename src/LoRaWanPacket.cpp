...
LoRaWanPacketClass::LoRaWanPacketClass() { setTimeout(0); }

int LoRaWanPacketClass::begin() { return 1; }
...
int LoRaWanPacketClass::available() { return payload_len - payload_position; }

int LoRaWanPacketClass::read() {
    return payload_position < payload_len ? payload_buf[payload_position++]
                                          : -1;
}
...
int LoRaWanPacketClass::length() { return available(); }
...
void LoRaWanPacketClass::join(const char *_deui, const char *_aeui,
                              const char *_akey) {
    setDevEui(_deui);
    setAppKey(_akey);
    setAppEui(_aeui);
...
void LoRaWanPacketClass::setAppEui(uint8_t *_aeui) { memcpy(AppEui, _aeui, 8); }

void LoRaWanPacketClass::setAppEui(const char *_aeui) {
    LORA_HEX_TO_BYTE((char *)AppEui, (char *)_aeui, 8);
}

void LoRaWanPacketClass::setDevEui(uint8_t *_deui) { memcpy(DevEui, _deui, 8); }

void LoRaWanPacketClass::setDevEui(const char *_deui) {
    LORA_HEX_TO_BYTE((char *)DevEui, (char *)_deui, 8);
}

void LoRaWanPacketClass::setJoinValuesFromRTC(
    uint8_t *_deui, uint8_t *_aeui, uint8_t *_akey, uint8_t *_daddr,
    uint8_t *_nwkSKey, uint8_t *_appSKey, uint32_t _frameCount,
    uint32_t _frameCountDown, uint8_t _FPort, uint8_t _FCtrl,
    uint8_t _lastMac) {
    memcpy(DevEui, _deui, 8);
    memcpy(AppEui, _aeui, 8);
    memcpy(AppKey, _akey, 16);
    memcpy(DevAddr, _daddr, 4);
    memcpy(NwkSKey, _nwkSKey, 16);
    memcpy(AppSKey, _appSKey, 16);
    frameCount = _frameCount;
    frameCountDown = _frameCountDown;
    FPort = _FPort;
    FCtrl = _FCtrl;
    lastMac = _lastMac;
}

void LoRaWanPacketClass::personalize(const char *_devAddr, const char *_nwkSKey,
                                     const char *_appSKey) {
    uint8_t devAddr[4];
    uint8_t nwkSKey[16];
    uint8_t appSKey[16];
...
// setPort
// ----------------------------------------------------------------------------
void LoRaWanPacketClass::setPort(uint8_t port) { FPort = port; }

// ----------------------------------------------------------------------------
// DevStatusReq = 0x06,       // u1:battery 0,1-254,255=?,
...
#ifdef LORAWAN_DEBUG
    if (debug) Serial.println("Check Mic Error");
#endif
...
int16_t LoRaWanPacketClass::decode(uint8_t *buf, uint8_t len) {
    ...
    // join decode
    if (buf[0] == 0x20) return decodeJoin(buf, len);
    // others decodes
...
int16_t LoRaWanPacketClass::decodePacket(uint8_t *buf, uint8_t len) {
    // check devid
    if (checkDev(buf, len) == false) return -1;
    ...
    if (checkMic(buf, len, NwkSKey)) {
        ...
        PayloadEncode((uint8_t *)(buf + mlength), payload_len, AppSKey, DevAddr,
                      count, dir);
...
int16_t LoRaWanPacketClass::decodeJoin(uint8_t *buf, uint8_t len) {
    ...
    if (JoinComputeMic(buf, len - 4, AppKey)) {
        for (int i = 0; i < 4; i++) DevAddr[3 - i] = buf[7 + i];
...
bool LoRaWanPacketClass::isJoin() {
    if (DevAddr[3] == 0 && DevAddr[2] == 0 && DevAddr[1] == 0 &&
        DevAddr[0] == 0)
        return true;
    return false;
}
...
int16_t LoRaWanPacketClass::JoinPacket() {
    ...
    for (uint8_t i = 0; i < 8; i++) payload_buf[i + 1] = AppEui[7 - i];

    for (uint8_t i = 0; i < 8; i++) payload_buf[i + 1 + 8] = DevEui[7 - i];
...
// We only need to send a gateway message upstream that looks like a node
// message.
//
...
// So message size should be lass than 128 bytes if Payload is limited to 64
// bytes.
// ----------------------------------------------------------------------------

int16_t LoRaWanPacketClass::encode() { return encoder(FPort); }
...
int16_t LoRaWanPacketClass::encoder(byte fport) {
    ...
    // -------------------------------
    // FPort, either 0 or 1 bytes. Must be != 0 for non MAC messages such as
    // user payload
    //
    ...
    // You can add any byte string below based on you personal choice of sensors
    // etc.
    //
    // Payload bytes in this example are encoded in the LoRaCode(c) format

    // we have to include the AES functions at this stage in order to generate
    // LoRa Payload.
    if (payload_len > 0) {
        uint8_t CodeLength =
            PayloadEncode((uint8_t *)(payload_buf + mlength), payload_len,
                          AppSKey, DevAddr, frameCount, 0);
    ...
    mlength += PayloadComputeMic((uint8_t *)(payload_buf), mlength, NwkSKey,
                                 frameCount, 0);
